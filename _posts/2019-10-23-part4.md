...
Since the kernels replace the original $m\times n$ X by $m\times m$, the linear kernel(No kernel) will incorporate more data if $n>>m$. On the other hand, if we have a lot of training examples, $m>>n$, than the f will have more information.
